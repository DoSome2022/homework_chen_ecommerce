...
// src/action/membershipTierActions/route.ts
"use server";
...
const tierSchema = z.object({
  level: z.enum(["FREE", "SILVER", "GOLD", "PLATINUM"]),
  name: z.string().min(2, "名稱至少 2 個字"),
  price: z.coerce.number().int().min(0, "價格必須為非負整數"),
  benefits: z.array(z.string().trim().min(1, "權益不可為空")).min(1, "至少填寫一項權益"),
  color: z.string().optional(),

  // 新增欄位
  durationMonths: z.coerce.number().int().min(0, "效期必須為非負整數").default(12),
  discountPercent: z.coerce.number().int().min(0, "折扣不可為負").max(100, "折扣不可超過 100").default(0),
  ...
  try {
    ...
    await db.membershipTier.create({
      data: {
        ...
        minSpendForUpgrade: validated.data.minSpendForUpgrade ?? null,
        autoUpgrade: validated.data.autoUpgrade,
      },
    });
...
export async function deleteMembershipTier(id: string) {
  ...
  try {
    ...
    const usageCount = await db.userMembership.count({
      where: { tierLevel: existing.level },
    });
